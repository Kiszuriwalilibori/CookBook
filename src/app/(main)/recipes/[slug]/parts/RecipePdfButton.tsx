...
//         if (!recipe.source) return `https://yourdomain.com/recipes/${slug}`;
//         if (recipe.source.url) return recipe.source.url;
//         if (recipe.source.book) return recipe.source.book;
//         if (recipe.source.title && recipe.source.author) return `${recipe.source.title} – ${recipe.source.author}`;
//         if (recipe.source.where) return recipe.source.where;
//         return `https://yourdomain.com/recipes/${slug}`;
//     };
...
//                 doc.setFont(undefined, "normal");
//                 recipe.ingredients.forEach(ing => {
//                     const normalizedIng = useCustomFont ? `${ing.quantity}${ing.unit ? ` ${ing.unit}` : ""} ${ing.name}` : normalizePolishText(`${ing.quantity}${ing.unit ? ` ${ing.unit}` : ""} ${ing.name}`);
//                     const ingText = `- ${normalizedIng}`;
//                     const ingLines = doc.splitTextToSize(ingText, maxLineWidth);
//                     ingLines.forEach((line: string) => {
//                         if (yPosition > 280) {
...
//             doc.setFont(undefined, "italic");
//             const zrodloText = useCustomFont ? "Źródło:" : "Zrodlo:";
//             doc.text(`${zrodloText} ${getSourceText()}`, 20, yPosition);
