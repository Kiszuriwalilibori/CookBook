...
// }
// Aggregates recipe fields from Sanity and upserts a single recipesSummary document.
// Protects against webhook-induced update loops by:
// - verifying an optional webhook secret header
// - ignoring incoming webhooks that are updates to the recipesSummary doc itself
// - only running aggregation when the webhook is for recipe documents (if payload includes _type)
//
// Aggregates recipe fields from Sanity and upserts a single recipesSummary document.
// Protects against webhook-induced update loops by verifying an optional webhook secret header
// and ignoring incoming updates to the summary itself.
// Requires SANITY_PROJECT_ID, SANITY_DATASET and SANITY_TOKEN (for writes) in environment.
...
const SANITY_TOKEN = process.env.SANITY_TOKEN || "";
const SANITY_WEBHOOK_SECRET = process.env.SANITY_WEBHOOK_SECRET || ""; // optional
...
export async function POST(req) {
    try {
        ...
        // Use the caught error variable so lint won't complain it is unused
        console.error("Aggregation/upsert error:", error);
        return new Response(JSON.stringify({ error: "Aggregation/upsert failed", details: error?.message ?? String(error) }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
}
