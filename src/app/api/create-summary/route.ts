...
export async function POST(req: NextRequest) {
    try {
        console.log("create-summary POST triggered");

        // Webhook secret check
        if (SANITY_WEBHOOK_SECRET_CREATE_SUMMARY) {
            const incomingSecret = (req.headers.get("x-webhook-secret") || req.headers.get("x-sanity-webhook-secret") || "").trim();
            console.log("Incoming webhook secret:", incomingSecret ? "present" : "missing");
            if (!incomingSecret || incomingSecret !== SANITY_WEBHOOK_SECRET_CREATE_SUMMARY) {
        ...
        if (incoming && typeof incoming["_type"] === "string" && incoming["_type"] !== "recipe") {
            console.log("Skipping non-recipe document");
        ...
        const resp = await fetch(url, fetchOpts);
        console.log("Recipe fetch status:", resp.status);
        if (!resp.ok) {
            const text = await resp.text();
            console.error("Sanity read failed:", resp.status, text);
            throw new Error(`Sanity read failed: ${resp.status} ${resp.statusText}`);
        }

        const payload = (await resp.json()) as SanityQueryResponse;
        console.log("Fetched recipes count:", Array.isArray(payload.result) ? payload.result.length : 0);
        const recipes: RecipeDoc[] = Array.isArray(payload.result) ? payload.result : [];

        // Build summary document (same as your current code)
        const summaryDoc: SummaryDoc = {
            _id: "summary",
            _type: "summary",
            totalCount: recipes.length,
            products: getUniqueSorted(new Set(recipes.flatMap(r => r.products?.map(normalizeLower).filter(Boolean) || []))),
            dietary: getUniqueSorted(new Set(recipes.flatMap(r => r.dietary?.map(normalizeLower).filter(Boolean) || []))),
            cuisine: getUniqueSorted(new Set(recipes.flatMap(r => (Array.isArray(r.cuisine) ? r.cuisine.map(normalizeLower).filter(Boolean) : typeof r.cuisine === "string" ? [normalizeLower(r.cuisine)].filter(Boolean) : [])))),
            tags: getUniqueSorted(new Set(recipes.flatMap(r => r.tags?.map(normalizeLower).filter(Boolean) || []))),
            title: getUniqueSorted(new Set(recipes.flatMap(r => (r.title ? [capitalizeFirst(r.title)].filter(Boolean) : [])))),
            "source.http": getUniqueSorted(new Set(recipes.flatMap(r => (r.source?.http ? [normalizeHttp(r.source.http)] : [])))),
            "source.book": getUniqueSorted(new Set(recipes.flatMap(r => (r.source?.book ? [normalizeFirstWordCapital(r.source.book)] : [])))),
            "source.title": getUniqueSorted(new Set(recipes.flatMap(r => (r.source?.title ? [normalizeFirstWordCapital(r.source.title)] : [])))),
            "source.author": getUniqueSorted(new Set(recipes.flatMap(r => (r.source?.author ? [normalizeAuthor(r.source.author)] : [])))),
            "source.where": getUniqueSorted(new Set(recipes.flatMap(r => (r.source?.where ? [normalizeWhere(r.source.where)] : [])))),
        };

        console.log("Summary document prepared:", summaryDoc);
        ...
        console.log("Mutate response status:", mutateResp.status);
        if (!mutateResp.ok) {
        ...
        const message = error instanceof Error ? error.message : String(error);
        console.error("create-summary failed:", message);
