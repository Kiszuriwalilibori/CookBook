...
describe("useCreateRecipeFilterFields", () => {
    const emptyOptions: RecipeFilter = {
        title: [],
        cuisine: [],
        tags: [],
        dietary: [],
    ...
    it("should return all base filter fields with correct structure", () => {
        ...
        expect(result.current[11].key).toBe("source.where");
    });
    ...
    it("should not populate options for switch component", () => {
        const { result } = renderHook(() => useCreateRecipeFilterFields(emptyOptions));
    ...
    //     const allKeys = result.current.map(f => f.key).sort();
    //     const combinedKeys = [...singleFields, ...multipleFields].sort();
    //     expect(combinedKeys).toEqual(allKeys);
    // });
    it("should have correct multiple flag for each field", () => {
        const { result } = renderHook(() => useCreateRecipeFilterFields(emptyOptions));
    ...
    it("should mark admin-required fields correctly", () => {
        ...
        expect(adminFields.map(f => f.key).sort()).toEqual(["kizia", "status", "source.url", "source.book", "source.title", "source.author", "source.where"].sort());
    });
    ...
    it("should return correct placeholder for dietary field", () => {
        const { result } = renderHook(() => useCreateRecipeFilterFields(emptyOptions));
        expect(result.current.find(f => f.key === "dietary")?.placeholder).toBe("np wegańska");
    });
